    ...
    def legacy_wifi_check_gw_aggregations(self, run_date):
        ...
        withColumn("signalstrength", func.when((col("device_deviceinfo_hardwareversion") == 'CODA-4582-1A') |
                                               ...
                                               func.when((col("x_comcast_com_rssi").isNotNull()) &
                                                         ...
                                                         (col("x_comcast_com_rssi") != 0), col("x_comcast_com_rssi")). \
        ...
        # 9
        filter_based_on_last_fiveteen_days = preprocessor.filter_records(connected_devices_hitron_host,
        ...
        # Check the number of polls in the last 3 days

        filter_based_on_last_three_days = preprocessor.filter_records(connected_devices_hitron_host,
                                                                  ["gateway_macaddress", "polling_date"],
                                                                  "in_three_date",
        ...
        filter(
        (connection_joined_fiveteen_three["percent_second_threshold_GW"].cast(DecimalType()) >= self.second_threshold)). \
        filter((connection_joined_fiveteen_three["percent_first_threshold_GW"].cast(DecimalType()) >= self.first_threshold)). \
        ...
        withColumn("polling_span", func.when((col("polling_timestamp") >=
                                             ...
                                             | (col("polling_timestamp") <= date_format(to_timestamp(lit("08:00:00"),
                                                                                                     "hh:mm:ss"),
                                                                                        'hh:mm:ss'))
                                             , "Overnight").otherwise("Daylight"))

        self.spark.sql("DROP TABLE IF EXISTS business_users_test.tmp_single_modem")
        ...
        connection_joined_fiveteen_three_new.select("gateway_macaddress",
                                                ...
                                                "percent_first_threshold_GW",
                                                "percent_second_threshold_GW"
                                                ).write.saveAsTable("business_users_test.tmp_single_modem")

        self.spark.sql("INSERT INTO TABLE business_users_test.tmp_single_modemss_v1 SELECT * from business_users_test.tmp_single_modem")
