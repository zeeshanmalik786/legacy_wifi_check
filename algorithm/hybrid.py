...
class hybrid:
    ...
    def hybrid(self):
        ...
        first = self.obj.get_data("business_users_test.tmp_single_modem_device_bounder", ["gateway_macaddress",
        ...
        withColumn("aggregation_date", func.date_sub(current_date(), self.aggregation_window_far)).\
        withColumn("aggregation_data_near", func.date_sub(current_date(), self.aggregation_window_near))

        second = self.obj.get_data("business_users_test.tmp_single_modem_device", ["gateway_macaddress",
                                                                          "associated_device_macaddress",
                                                                          "interface",
                                                                          "unified_ssid",
                                                                          "signalstrength",
        ...
        withColumn("percent_outlier_far", func.when((func.col("sum_of_outlier_far") / self.aggregation_window_far) >= 1, 1).\
        ...
        withColumnRenamed("sum(less_than_80dbm)", "sum_of_less_80dbm").\
        withColumn("percent_outlier_near", func.when((func.col("sum_of_outlier_near") / self.aggregation_window_near) >= 1, 1).\
                   otherwise((func.col("sum_of_outlier_near")/self.aggregation_window_near))).\
        withColumn("percent_80dbm_near", func.when((func.col("sum_of_less_80dbm") / self.aggregation_window_near) >= 1, 1).\
        ...
        self.spark.sql("DROP TABLE IF EXISTS business_users_test.tmp_single_modems_v1")
        ...
        join_first_frame_1_frame_2 = join_first_frame_1_frame_2.select("gateway_macaddress",
            ...
            withColumn("norm_signalstrength", (func.when(((col("signalstrength") - (-60)) / ((-80) - (-60))) > 1, 1).
                                               otherwise(func.when(((col("signalstrength") - (-60)) / ((-80) - (-60))) < 0, 0).
        ...
        withColumn("distributor_percent_outlier_near", func.when(col("percent_outlier_near") == 0, (self.weights_percent_outlier_near/self.number_of_features)).otherwise(0)).\
        withColumn("distributor_percent_outlier_far",   func.when(col("percent_outlier_far") == 0,  (self.weights_percent_outlier_far/self.number_of_features)).otherwise(0)).\
            withColumn("distributor_percent_80dbm_near",  func.when(col("percent_80dbm_far") == 0,   (self.weights_percent_80dbm_far/self.number_of_features)).otherwise(0)).\
            withColumn("distributor_percent_80dbm_far",  func.when(col("percent_80dbm_near") == 0,  (self.weights_percent_80dbm_near/self.number_of_features)).otherwise(0))

        output = output.withColumn("device_score", func.when((col("missing_data_device") == "Yes") & (col("interface") != "Ethernet"),
                                             func.round((100 * (1 - (col("norm_signalstrength") * 1))))).\
                   otherwise(func.when(col("interface") == "Ethernet", "Not Applicable").\
                   otherwise(func.round((100 * (1 - ((col("percent_outlier_near") * (self.weights_percent_outlier_near +  col("distributor_percent_outlier_near") +
                                                                                            col("distributor_percent_outlier_far") +
                                                                                            col("distributor_percent_80dbm_near")   +
                                                                                            col("distributor_percent_80dbm_far")))
                                                      +
                                                     (col("percent_outlier_far") * (self.weights_percent_outlier_far + col("distributor_percent_outlier_near") +
                                                                                            col("distributor_percent_outlier_far") +
                                                                                            col("distributor_percent_80dbm_near")   +
                                                                                            col("distributor_percent_80dbm_far")))
                                                     +
                                                     (col("percent_80dbm_far") * (self.weights_percent_80dbm_far + col("distributor_percent_outlier_near") +
                                                                                            col("distributor_percent_outlier_far") +
                                                                                            col("distributor_percent_80dbm_near")   +
                                                                                            col("distributor_percent_80dbm_far")))
                                                     +
                                                     (col("percent_80dbm_near") * (self.weights_percent_80dbm_near + col("distributor_percent_outlier_near") +
                                                                                            col("distributor_percent_outlier_far") +
                                                                                            col("distributor_percent_80dbm_near")   +
                                                                                            col("distributor_percent_80dbm_far")))))), 2).alias("device_score"))))
        output = output.withColumn("device_score", func.when(((col("percent_outlier_near") == 0) &
                                                             ...
                                                             (col("percent_80dbm_far") == 0)),((100 - (col("norm_signalstrength") * 100)))).otherwise(col("device_score")))
        output = output.\
        withColumn("Rating", func.when((col("device_score") <= 30), "Intermittent Device").
                   otherwise(func.when((col("device_score") > 30) & (col("device_score") <= 40), "Bad Device").
                             otherwise(func.when((col("device_score") <= 80) & (col("device_score") > 40),"Good Device").
                                       otherwise(func.when((col("device_score") > 80), "Excellent Device")))))

        output.\
            withColumn("Rating", func.when(col("device_score") == "Not Applicable","None").otherwise(col("Rating"))).\
            write.saveAsTable("business_users_test.tmp_single_modems_v1")
